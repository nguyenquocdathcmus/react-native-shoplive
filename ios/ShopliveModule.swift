...
class ShopliveModule: RCTEventEmitter {
  ...
  func playWithAuthToken(_campaign: String, jwtToken: String) {
     DispatchQueue.main.async {
  ...
  func setShareUrl(_ schemeUrl: String) {
    DispatchQueue.main.async {
      //share api changed since v1.5.13
...
class ShopLiveHandler: NSObject, ShopLiveSDKDelegate {
  ...
  func handleReceivedCommand(_ command: String, with payload: Any?) {
     let payloadString = String(describing: payload)
        ...
        print("Error: Emitter is not initialized")
        return
        }

        emitter.sendEvent(withName: "onReceivedCommand", body: ["command": command, "data": payloadString])
